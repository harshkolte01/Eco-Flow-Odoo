...
export default function EditRulePage() {
  ...
  const { rule, loading, error: loadError } = useRule({ ruleId, autoFetch: true });
  ...
  const handleSubmit = async (formData: RuleFormData) => {
    ...
    try {
      ...
      // Redirect to the rule's detail page
      router.push(`/settings/approval-rules/${ruleId}`);
    } catch (err) {
      const message = err instanceof ApiError ? err.message : 'Failed to update approval rule';
      setError(message);
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="mt-3 text-sm text-gray-500">Loading rule...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="mx-auto max-w-5xl px-4 py-8">
        <div className="rounded-md bg-red-50 p-4">
          <div className="flex">
            <div className="ml-3">
              <h3 className="text-sm font-medium text-red-800">{loadError || 'Rule not found'}</h3>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Approval Rule</h1>
      ...
      {error && (
        <div className="mb-6 rounded-md bg-red-50 p-4">
          <div className="flex">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-red-800">Error updating rule</h3>
